...
await send('Runtime.evaluate', { expression: `document.querySelector('.research-verdict button')?.click()` })
await new Promise(resolve => setTimeout(resolve, 250))
const research = await send('Runtime.evaluate', { expression: `JSON.stringify({researchVisible:!!document.querySelector('.kpis'),rejectedText:document.querySelector('.live-source strong')?.textContent})`, returnByValue: true })
console.log(before.result.value)
console.log(after.result.value)
console.log(research.result.value)
