...
export function HistoryTable({ trades }: { trades: TradeRecord[] }) {
  return <section className="panel history-panel">
    <div className="panel-heading"><div><h2>Riwayat trade</h2><span>20 terbaru · simulasi setelah biaya</span></div><span>{trades.length} sampel</span></div>
    <div className="table-wrap"><table>
      ...
      <tbody>{trades.slice().reverse().slice(0, 20).map((t, i) => <tr key={`${t.date}-${t.ticker}-${i}`}>
