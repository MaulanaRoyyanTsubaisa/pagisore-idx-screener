...
import { compactIdr, idr, number } from '../lib/format'
...
export function SignalTable({ signals, loading, onSelect }: { signals: Signal[]; loading: boolean; onSelect: (s: Signal) => void }) {
  return <section className="panel signal-panel">
    ...
    <div className="table-wrap">
      <table>
        <thead><tr><th>#</th><th>Ticker</th><th>Harga</th><th>Waktu</th><th>Bid/Offer</th><th>Nilai transaksi</th><th>Range entry</th><th>Target</th><th>Stop</th><th>Skor</th><th>Status</th></tr></thead>
        <tbody>
          {loading && <tr><td colSpan={11}><div className="loading-row">Mengambil snapshot pasar…</div></td></tr>}
          {!loading && signals.map((s, i) => <tr key={s.ticker} onClick={() => onSelect(s)} className="clickable-row">
            ...
            <td>{compactIdr(s.value)}</td><td>{number(s.entryLow)}–{number(s.entryHigh)}</td><td>{idr(s.target)}</td><td>{idr(s.stop)}</td>
          ...
          {!loading && !signals.length && <tr><td colSpan={11}><div className="empty-state"><AlertTriangle size={22} /><strong>Belum ada saham yang lolos</strong><span>Coba longgarkan filter atau gunakan data demo untuk melihat alurnya.</span></div></td></tr>}
        </tbody>
      </table>
    </div>
