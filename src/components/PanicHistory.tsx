    ...
    {loading && !days.length ? <div className="history-empty">Memuat histori…</div> : !days.length ? <div className="history-empty">Belum ada sesi historis yang tersimpan.</div> : <div className="history-days">
      {days.map(day => {
        ...
        return <article className="history-day" key={day.date}>
          ...
          {expanded && <div className="history-trades">{day.candidates.length ? day.candidates.map(row => <div className="history-trade" key={`${day.date}-${row.ticker}`}><b>#{row.rank} {row.ticker} <i className={`tier tier-${row.qualityTier.toLowerCase()}`}>TIER {row.qualityTier}</i></b><span>turun {pct(row.changePct)}</span><span>limit −{row.entryDiscountPct}% · {idr(row.entry)}</span><span>{row.filled ? `close ${idr(row.close)}` : `low ${idr(row.low)} > limit`}</span><strong className={!row.filled ? 'skipped' : Number(row.netPct) > 0 ? 'positive' : 'negative'}>{row.filled ? pct(Number(row.netPct), true) : 'SKIP · Rp0'}</strong></div>) : <div className="history-empty compact">Tidak ada kandidat · SKIP · Rp0</div>}</div>}
        </article>
      })}
    </div>}
